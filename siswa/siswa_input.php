          ...
          if (isset($_POST['btn-save'])) {
            ...
            if (count($results)>0 && $nomor_peserta == $results['nomor_peserta'] || $nomor_ijazah == $results['nomor_ijazah']) {
              ...
              // Cek apakah tipe file yang di upload adalah JPG/JPEG/PNG
                if ($tipe_file == "image/jpeg" || $tipe_file == "image/png") {
                  ...
                  if ($ukuran_file <= 1000000) {
                    # jika ukuran file kurang dari sama dengan 2MB, lakukan:
                      ...
                      // cek data nis 
                      $stmt = $DB_con->prepare('SELECT * FROM tbl_siswa WHERE id_user=:id_user');
                      $stmt->bindparam(':id_user',$_POST['id_user']);
                      $stmt->execute();
                      $result = $stmt->fetch(PDO::FETCH_ASSOC);
                      if ($result['id_user']==$id_user) {
                          echo "<script> alert('User dah ada') </script>";
                          echo "<meta http-equiv='refresh' content='0; url=?module=siswa_input'>";
                      }else{
                        // proses upload
                        ...
                        $siswa->create($nis,$nisn,$nama_siswa,$alamat,$no_telp,$tempat_lahir,$tgl_lahir,$nama_orang_tua,$sekolah_asal,$nomor_peserta,$tahun_lulus,$kepala_sekolah,$nomor_ijazah,$nilai_rata_rata,$nama_jurusan,$keterangan,$nama_file,'admin',$id_user,$kondisi,$detail_kondisi);
                        echo "<script> alert('Siswa Berhasil Di Tambah') </script>";
                ...
                $result = $stmt->fetch(PDO::FETCH_ASSOC);
                if ($result['id_user']==$id_user) {
                    echo "<script> alert('NUser dah ada sudah ada') </script>";
                    echo "<meta http-equiv='refresh' content='0; url=?module=siswa_input'>";
                }else{
          ...
          <div class="col-md-12">
            <form method="POST" enctype="multipart/form-data" action="">
              <table class="table table-bordered">
                ...
                <tr>
                  <td>Nama</td>
                  <td><input class="form-control" type="text" name="nama_siswa" placeholder="Nama.." required></td>
                </tr>
                <tr>
                  <td>Alamat Tinggal</td>
                  <td><textarea class="form-control" placeholder="Alamat Tinggal" name="alamat"></textarea></td>
                </tr>
                <tr>
                  <td>No.Telp</td>
                  <td><input class="form-control" type="text" name="no_telp" placeholder="Nomor Telp/Whatsapp.." required></td>
                </tr>
                <tr>
                  <td>Jurusan</td>
                  <td>
                ...
                <tr>
                  ...
                  <td><input class="form-control" type="text" placeholder="Nomor Peserta Ujian.." name="nomor_peserta"></td>
                </tr>
                <tr>
                  <td>Tahun Lulus</td>
                ...
                <tr>
                  <td>Nilai Akhir</td>
                  <td><input class="form-control" type="text" placeholder="Nilai Akhir" readonly value = "0" name="nilai_rata_rata"></td>
                </tr>
                <tr>
                <tr>
                  <td>Kondisi</td>
                ...
                <tr>
                  ...
                  <td><textarea class="form-control" placeholder="Keterampilan/Skill" name="keterangan"></textarea></td>
                  <td><input class="form-control" type="hidden" placeholder="id_user" name="id_user" value="<?php echo $id_user ?>"></td>
                </tr>
                <tr>
                  <td>Foto</td>
                  <td><input type="file" name="foto">
                  <font color="red">*ukuran foto max 2MB, *contoh format nama file: tahunlulus_jurusan_nama.jpg/png</font></td>
                </tr>
                <tr>
